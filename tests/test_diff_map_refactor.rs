...
fn test_changedmap_has_structured_output() {
    ...
    assert_eq!(
        detail.old_values.x_axis.len(),
        6,
        "Old X axis should have 6 points"
    );
    assert_eq!(
        detail.old_values.y_axis.len(),
        4,
        "Old Y axis should have 4 points"
    );
    assert_eq!(
        detail.new_values.x_axis.len(),
        5,
        "New X axis should have 5 points"
    );
    assert_eq!(
        detail.new_values.y_axis.len(),
        3,
        "New Y axis should have 3 points"
    );

    // Verify values are accessible
    assert_eq!(
        detail.old_values.values.len(),
        24,
        "Old values should have 24 elements (6x4)"
    );
    assert_eq!(
        detail.new_values.values.len(),
        15,
        "New values should have 15 elements (5x3)"
    );
}
...
fn test_changedmap_json_is_clean() {
    ...
    assert!(
        !json_output.contains("\\\""),
        "JSON should not contain escaped quotes"
    );

    // The JSON should contain dimension fields
    assert!(
        json_output.contains("\"dim_x\":"),
        "JSON should have dim_x field"
    );
    assert!(
        json_output.contains("\"dim_y\":"),
        "JSON should have dim_y field"
    );
    assert!(
        json_output.contains("\"values\": ["),
        "JSON should have values array"
    );
    assert!(
        json_output.contains("\"x_axis\": ["),
        "JSON should have x_axis array"
    );
    assert!(
        json_output.contains("\"y_axis\": ["),
        "JSON should have y_axis array"
    );
}
...
fn test_map_change_detail_populated() {
    ...
    assert!(
        !map_diff.old_values.x_axis_name.is_empty(),
        "Old X axis name should be populated"
    );
    assert!(
        !map_diff.old_values.y_axis_name.is_empty(),
        "Old Y axis name should be populated"
    );
    assert!(
        !map_diff.new_values.x_axis_name.is_empty(),
        "New X axis name should be populated"
    );
    assert!(
        !map_diff.new_values.y_axis_name.is_empty(),
        "New Y axis name should be populated"
    );

    // Verify attrs exist
    assert!(
        !map_diff.old_values.attrs.is_empty(),
        "Old attrs should be populated"
    );
    assert!(
        !map_diff.new_values.attrs.is_empty(),
        "New attrs should be populated"
    );

    // Check for LANGNAME change
    let old_langname = map_diff
        .old_values
        .attrs
        .iter()
        .find(|a| a.identifier == "LANGNAME")
        .map(|a| a.value.clone());
    let new_langname = map_diff
        .new_values
        .attrs
        .iter()
        .find(|a| a.identifier == "LANGNAME")
    ...
    assert_eq!(
        new_langname,
        Some("Modified 2D calibration map".to_string())
    );
}
...
fn test_2d_array_values_accessible() {
    ...
    if let Some(ref values_2d) = map_diff.old_values.values_2d {
        ...
        assert_eq!(
            values_2d[0].len(),
            6,
            "Old 2D values first row should have 6 columns"
        );
    }
    ...
    if let Some(ref values_2d) = map_diff.new_values.values_2d {
        ...
        assert_eq!(
            values_2d[0].len(),
            5,
            "New 2D values first row should have 5 columns"
        );
    }
}
