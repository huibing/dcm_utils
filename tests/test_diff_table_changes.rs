...
fn test_diff_detects_1d_table_dim_change() {
    ...
    let var_0014_changed = diff
        .iter()
        .any(|d| matches!(d, dcm_utils::diff::DcmDiff::Changed { name, .. } if name == "VAR_0014"));
    assert!(
        var_0014_changed,
        "Should detect VAR_0014 as changed (1D table dimension changed from 9 to 7)"
    );

    // Verify the old and new values are different
    let var_0014_diff = diff.iter().find(
        |d| matches!(d, dcm_utils::diff::DcmDiff::Changed { name, .. } if name == "VAR_0014"),
    );
...
fn test_diff_detects_2d_table_dim_change() {
    ...
    let var_0304_changed = diff.iter().any(
        |d| matches!(d, dcm_utils::diff::DcmDiff::ChangedMap { name, .. } if name == "VAR_0304"),
    );
    assert!(
        var_0304_changed,
        "Should detect VAR_0304 as changed (2D table dimension changed from 16x9 to 12x9)"
    );
}
...
fn test_diff_detects_axis_var_name_change_for_1d_table() {
    ...
    assert!(
        var_0014_changed,
        "Should detect VAR_0014 as changed (axis_var_name changed from VAR_0013 to VAR_0239)"
    );
}
...
fn test_diff_json_contains_table_change_details() {
    ...
    assert!(
        json_output.contains("VAR_0014"),
        "JSON should contain VAR_0014"
    );
    assert!(
        json_output.contains("Changed"),
        "JSON should contain 'Changed' variant for table changes"
    );

    // Verify old and new values are present
    assert!(
        json_output.contains("\"old\""),
        "JSON should contain 'old' field"
    );
    assert!(
        json_output.contains("\"new\""),
        "JSON should contain 'new' field"
    );
}
    ...
    if let Some(dcm_utils::diff::DcmDiff::Changed { old, new, .. }) = var_0014_diff {
        ...
        match old {
            dcm_utils::value::Value::WERT(v) => {
                assert!(
                    (v[0] - 320.0).abs() < 0.001,
                    "First old value should be 320.0"
                );
                assert!(
                    (v[8] - 1600.0).abs() < 0.001,
                    "Last old value should be 1600.0"
                );
            }
